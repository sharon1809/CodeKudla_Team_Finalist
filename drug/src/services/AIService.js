...
class AIService {
  ...
  async callLLM(systemPrompt, userPrompt, maxTokens = 1200) {
  ...
  async generateSafetyReport(patient, drug, safetyCheck) {
  ...
  async generateDetailedReport(safetyCheckResult) {
  ...
  /**
   * AI FINAL DECISION — the model adjudicates the safety status from the live
   * interaction screening data. No manual/heuristic severity thresholds are
   * used; the clinician reviews the interaction evidence and the AI verdict.
   * @param {{patient: Object, drug: Object, allergyCheck: string|null, interactionReport: Object}} input
   * @returns {Promise<{status: 'SAFE'|'WARNING'|'BLOCKED', message: string, summary: string, recommendations: string[]}>}
   */
  async generateInteractionDecision({ patient, drug, allergyCheck, interactionReport }) {
  ...
  buildDecisionPrompt({ patient, drug, allergyCheck, interactionReport }) {
    return `
...
`;
  ...
  buildPrompt(patient, drug, safetyCheck) {
    return `
Patient Information:
- Name: ${patient.name}
- Age: ${patient.age}
...
- Current Medications: ${patient.currentMedications && patient.currentMedications.length ? patient.currentMedications.map(m => m.drugName).join(', ') : 'None'}
